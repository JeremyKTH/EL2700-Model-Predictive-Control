...
class Astrobee(object):
    ...
    def one_axis_ground_dynamics(self):
        ...
        """Question 1"""
        Ac[0,1] = 1 #Given state space and control matrices
        Bc[1] = 1/self.mass
    ...
    def casadi_c2d(self, A, B, C, D):
        ...
        x = ca.MX.sym('x', A.shape[1]) #making the state and input vectors, with variable x and 1 column
        ...
        Ad = ca.Function('jac_x_Ad', [x, u], [ca.jacobian(self.Integrator(x0=x, p=u)['xf'], x)])
        Bd = ca.Function('jac_u_Bd', [x, u], [ca.jacobian(self.Integrator(x0=x, p=u)['xf'], u)])
    ...
    def set_discrete_dynamics(self, Ad, Bd, Cd, Dd):
        """
    ...
    def poles_zeros(self, Ad, Bd, Cd, Dd):
        ...
        sys = control.StateSpace(Ad, Bd, Cd, Dd, dt=self.dt) #discrete
        control.pzmap(sys)
        plt.show()
