...
class Controller(object):
    ...
    def update_integral(self, x):
        ...
        #self.i_term = 0.0
        self.i_term = self.i_term + self.dt*(x[0] - 0.0)

    def reset_integral(self): #anti-windup of integrator term
        """
        Reset the integral action of the controller
        """
        self.i_term = 0.0

    def set_integral_gain(self, ki): #where we input the proportional gain control we want
        """
    ...
    def control_law(self, x):
        ...
        #u = 0.0
        u = -self.L@x - self.i_term*self.Ki #New control law added
        return u
