...
class EmbeddedSimEnvironment(object):
    ...
    def run(self, x0):
        ...
        for i in range(sim_loop_length):
            ...
            #Add noise: 1:01 --> h = 0.1m so the system can jump 2mm in 0.1s
            
            p_noise = np.random.uniform(-0.002,0.002, (3,1))
            v_noise = np.random.uniform(-0.002,0.002, (3,1))
            q_noise = np.random.uniform(-0.01,0.01, (4,1))
            w_noise = np.random.uniform(-0.002,0.002, (3,1))
            noise_vec = np.concatenate((p_noise, v_noise, q_noise, w_noise), axis=0)
            x_next = x_next + noise_vec
    ...
    def visualize_error(self):
        ...
        ax1.plot(t, x_vec[0, :], 'r--',
                 t, x_vec[1, :], 'g--',
                 t, x_vec[2, :], 'b--')
        ax1.axhline(0.05,color = "blue", ls="-.", label="Uppper Lim x", linewidth=1)
        ax1.axhline(-0.05,color = "blue", ls="-.", label="Lower Lim x", linewidth=1)
        ax1.legend(["x1", "x2", "x3"])
        ...
        ax3.plot(t, x_vec[6, :], 'r--',
                 t, x_vec[7, :], 'g--',
                 t, x_vec[8, :], 'b--')
        ax3.axhline(10,color = "blue", ls="-.", label="Uppper Lim x", linewidth=1)
        ax3.axhline(10,color = "blue", ls="-.", label="Lower Lim x", linewidth=1)
        ax3.legend(["ex", "ey", "ez"])
