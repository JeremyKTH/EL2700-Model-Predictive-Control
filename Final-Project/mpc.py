"""
Model Predictive Control - CasADi interface
Adapted from Helge-Andre Langaker work on GP-MPC
Customized by Pedro Roque for EL2700 Model Predictive Countrol Course at KTH
"""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function

import time
import numpy as np
import casadi as ca
import casadi.tools as ctools
import yaml
from util import *


class MPC(object):

    def __init__(self, model, dynamics,
                 param='P1', N=10, trajectory_tracking=False,
                 ulb=None, uub=None, xlb=None, xub=None,
                 terminal_constraint=None, tuning_file=None,
                 solver_opts=None):
        """
        Constructor for the MPC class.

        :param model: System model
        :type model: Astrobee
        :param dynamics: Astrobee dynamics model
        :type dynamics: ca.Function
        :param N: horizion length
        :type N: int
        :param Q: state weight matrix, defaults to None
        :type Q: np.ndarray, optional
        :param P: terminal state weight matrix, defaults to None
        :type P: np.ndarray, optional
        :param R: control weight matrix, defaults to None
        :type R: np.ndarray, optional
        :param ulb: control lower bound vector, defaults to None
        :type ulb: np.ndarray, optional
        :param uub: control upper bound vector, defaults to None
        :type uub: np.ndarray, optional
        :param xlb: state lower bound vector, defaults to None
        :type xlb: np.ndarray, optional
        :param xub: state upper bound vector, defaults to None
        :type xub: [type], optional
        :param terminal_constraint: terminal constriant polytope, defaults to None
        :type terminal_constraint: Polytope, optional
        :param solver_opts: additional solver options, defaults to None.
                            solver_opts['print_time'] = False
                            solver_opts['ipopt.tol'] = 1e-8
        :type solver_opts: dictionary, optional
        """

        # State flags
        self.fw_propagating = False
        self.trajectory_tracking = trajectory_tracking

        build_solver_time = -time.time()
        self.model = model
        self.dt = model.dt
        self.Nx, self.Nu = model.n, model.m #Dimension of state, control input
        self.Nt = N
        print("Horizon steps (time): ", N * self.dt)
        self.dynamics = dynamics

        # Initialize variables
        self.set_cost_functions_quat() #cost function
        self.set_slack_cost()
        self.x_sp = None

        # Cost function weights
        Q, R, P = self.load_params(param, tuning_file) #

        
        self.Q = ca.MX(Q)
        self.P = ca.MX(P)
        self.R = ca.MX(R)
        
        #create our state constraints and control constraints
        #These lines makes it so, if you dont set any arguments for constraints, it will default to no limits at all.
        #Safety guarantee, without constraints the solver cannot build
        if xub is None:
            xub = np.full((self.Nx), np.inf)
        if xlb is None:
            xlb = np.full((self.Nx), -np.inf)
        if uub is None:
            uub = np.full((self.Nu), np.inf)
        if ulb is None:
            ulb = np.full((self.Nu), -np.inf)

        # Starting state parameters - add slack here
        x0 = ca.MX.sym('x0', self.Nx)   #x0 is not always needed to be defined, but it most cases it is.
        if self.trajectory_tracking:
            # TODO: remove 'raise NotImplementedError' and create the desired symbolic param
            #raise NotImplementedError
            #x_ref = ca.MX.sym('x_ref', 13*(self.Nt + 1),) 
            x_ref = ctools.struct_symMX([(ctools.entry('x_ref', shape=(13,), repeat=self.Nt+1),
                                        )])
            
        else:
            x_ref = ca.MX.sym('x_ref', self.Nx,)    #reference can be static or updated
        u0 = ca.MX.sym('u0', self.Nu)
        param_s = ca.vertcat(x0, x_ref, u0) #Parameters that the solver needs to be created, init the solver.

        # Create optimization variables
        #we could do x = ca.MX.sym('x', self.Nx * self.Nt +1) aswell
        #same thing for u, but then it would be more messy to acces these variables.
        opt_var = ctools.struct_symMX([(ctools.entry('u', shape=(self.Nu,), repeat=self.Nt),
                                        ctools.entry('x', shape=(self.Nx,), repeat=self.Nt + 1),
                                        ctools.entry('lmbd', shape =(self.Nx), repeat = self.Nt)
                                        )])
                               
        self.opt_var = opt_var
        self.num_var = opt_var.size

        # Decision variable boundries
        #We will put explicit bounds of the components of the ctools structure above, thats why we dont set the constraints below.
        self.optvar_lb = opt_var(-np.inf)
        self.optvar_ub = opt_var(np.inf)

        # Set initial values
        obj = ca.MX(0)     #cost function objective starts at zero
        con_eq = []        
        con_ineq = []
        con_ineq_lb = []
        con_ineq_ub = []
        con_eq.append(opt_var['x', 0] - x0) #Setting constraint for x0, so it will optimize for the next step, otherwise it would optimize this aswell.

        # Generate MPC Problem, this runs offline, so computatation time doesnt matter.
        for t in range(self.Nt): #Nt horizon
            
            # Get variables
            x_t = opt_var['x', t] #My x_t is x at time t
            if self.trajectory_tracking:
                # TODO: remove 'raise NotImplementedError' and obtain the desired step in the reference trajectory
                x_r = x_ref['x_ref',t]
                #raise NotImplementedError
            else:
                x_r = x_ref         #xref is static, so we track a static setpoint
            u_t = opt_var['u', t]   #u_t is u at time t
            lmbd = opt_var['lmbd', t]

            # Dynamics constraint
            x_t_next = self.dynamics(x_t, u_t) #get our state at this step, our control input that we will optimize
            con_eq.append(x_t_next - opt_var['x', t + 1]) #set constraint that our state at next time step will be x_t_next, #lb< g(x,u) <ub

            # Input constraints
            if uub is not None:
                con_ineq.append(u_t)
                con_ineq_ub.append(uub)
                con_ineq_lb.append(np.full((self.Nu,), -ca.inf))
            if ulb is not None:
                con_ineq.append(u_t)
                con_ineq_ub.append(np.full((self.Nu,), ca.inf))
                con_ineq_lb.append(ulb)

            #x_t - lmbd < xub --> x_t < xub + lmbd
            #xlb < x_t + lmbd -- > xlb - lmbd < x_t    

            # State constraints, put slack variable in here
            if xub is not None:
                con_ineq.append(x_t-lmbd) #x_t - lmdb < xub --> x_t < xub + lmdb
                #con_ineq.append(x_t)
                con_ineq_ub.append(xub)
                con_ineq_lb.append(np.full((self.Nx,), -ca.inf))
            if xlb is not None:
                con_ineq.append(x_t + lmbd) #xt + lmbd #xlb < x_t + lmbd -> xlb - lmdb < x_t
                #con_ineq.append(x_t)
                con_ineq_ub.append(np.full((self.Nx,), ca.inf))
                con_ineq_lb.append(xlb)

                #xlb < x_t - lmbd < xub
               
            # Objective Function / Cost Function
            obj += self.running_cost(x_t, x_r, self.Q, u_t, self.R) #
            obj += self.slack_cost(lmbd)
        # Add terminal cost at the end, Terminal Cost
        obj += self.terminal_cost(opt_var['x', self.Nt], x_r, self.P)
        #obj +=self.running_cost(x_t,x_r,self.P, ca.MX.zeros(6,), ca.MX.zerios((6,6))) would work aswell
        #for slack:
        #obj += self.slack_cost(lmbd)

        # Terminal contraint, we use nonlinear mpc how can we enforce the terminal constraint.
        if terminal_constraint is not None:
            # Should be a polytope
            H_N = terminal_constraint.A
            if H_N.shape[1] != self.Nx:
                print("Terminal constraint with invalid dimensions.")
                exit()

            H_b = terminal_constraint.b
            con_ineq.append(ca.mtimes(H_N, opt_var['x', self.Nt]))
            con_ineq_lb.append(-ca.inf * ca.DM.ones(H_N.shape[0], 1))
            con_ineq_ub.append(H_b)

        # Equality constraints bounds are 0 (they are equality constraints),
        # -> Refer to CasADi documentation
        num_eq_con = ca.vertcat(*con_eq).size1() #calculating how many equality constraints we have
        num_ineq_con = ca.vertcat(*con_ineq).size1()    #how many ineq constr. we have
        con_eq_lb = np.zeros((num_eq_con,))   # setting the right hand side of the equality constraint so its zeros, see line 139
        con_eq_ub = np.zeros((num_eq_con,))   #set to zero so you for for example 0 < g < 0 for g to zero

        # Set constraints
        con = ca.vertcat(*(con_eq + con_ineq))
        self.con_lb = ca.vertcat(con_eq_lb, *con_ineq_lb)
        self.con_ub = ca.vertcat(con_eq_ub, *con_ineq_ub)

        # Build NLP Solver (can also solve QP)
        nlp = dict(x=opt_var, f=obj, g=con, p=param_s) #obj holds the cost we want to minimize, hold constraints con, param_s the external paramters the solver will recieve
        options = {
            'ipopt.print_level': 0,
            'print_time': False,
            'verbose': False,
            'expand': True
        }
        if solver_opts is not None:
            options.update(solver_opts)
        self.solver = ca.nlpsol('mpc_solver', 'ipopt', nlp, options) #great solver, this is the solver which will solve the problem. nlp is the dictionary that contains the solver

        build_solver_time += time.time()
        print('\n________________________________________')
        print('# Time to build mpc solver: %f sec' % build_solver_time)
        print('# Number of variables: %d' % self.num_var)
        print('# Number of equality constraints: %d' % num_eq_con)
        print('# Number of inequality constraints: %d' % num_ineq_con)
        print('----------------------------------------')
        pass

    def load_params(self, param, tuning_file=None):
        """
        Parameters loader function.
        Loads yaml parameters to generate Q, R and P.

        :param param: parameter setting ('P1', 'P2', 'P3')
        :type param: string
        """

        if param not in ['P1', 'P2', 'P3', 'P4','P5','P6']:
            print("Wrong param option. Select param='P1' or 'P2' or 'P3'.")
            exit()

        if tuning_file is None:
            f_path = "tuning.yaml"
        else:
            f_path = tuning_file

        with open(f_path, 'r') as stream:
            parameters = yaml.safe_load(stream)

            # Create numpy diags
            Q_diag = np.asarray(parameters[param]['Q'])
            R_diag = np.asarray(parameters[param]['R'])
            P_diag = parameters[param]['P_mult'] * Q_diag

            # Get matrices
            Q = np.diag(Q_diag)
            R = np.diag(R_diag)
            P = np.diag(P_diag)

            return Q, R, P

        raise NotADirectoryError("Wrong directory for yaml file.")

    def set_cost_functions_quat(self):
        """
        Helper method to create CasADi functions for the MPC cost objective.
        """
        # Create functions and function variables for calculating the cost
        Q = ca.MX.sym('Q', self.Nx - 1, self.Nx - 1) #-1 because of quaternion..., see video
        P = ca.MX.sym('P', self.Nx - 1, self.Nx - 1)
        R = ca.MX.sym('R', self.Nu, self.Nu) #Same dim as control matrix

        #Symbolic variables for state, reference xr and control input
        x = ca.MX.sym('x', self.Nx) 
        xr = ca.MX.sym('xr', self.Nx) #Trying to track position and velocity, thats why we have same dim as x. But it could be any other dim depending on what you track.
        u = ca.MX.sym('u', self.Nu)

        # Prepare variables
        p = x[0:3]  #position
        v = x[3:6]  #vel
        q = x[6:10] #quaternion
        w = x[10:]  #angular vel

        pr = xr[0:3]
        vr = xr[3:6]
        qr = xr[6:10]
        wr = xr[10:]

        # Calculate errors
        ep = p - pr #error in pos, pr-p would work aswell since squared in cost function.
        ev = v - vr #error i vel
        ew = w - wr #error in ang vel

        #1. This error is maximum 1, always less than 1 and 0 iff qr = q, 1 at maximum error
        # q=[qx qy qz qw] and qr=[qrx qry qrz qrw], quaternion distance
        #Or probably more like attitude error?
        eq = ca.MX.ones(3, 1) * (1 - ca.mtimes(qr.T, q)**2) #scaled with an quaternion error (quaternion distance)
        #Its just a measure of how far apart two quaternions are, its corralated with the angular distance of two quaternions
        #Max error 180 degrees, when quaternions point in different directions
        #This could also be eq = t - tr

        e_vec = ca.vertcat(*[ep, ev, eq, ew])

        # Calculate running cost, e'*Q*e + u'*R*u
        ln = ca.mtimes(ca.mtimes(e_vec.T, Q), e_vec) \
            + ca.mtimes(ca.mtimes(u.T, R), u)

        #Running cost, use casadi for more optimal calculation, this just define the function with its arguments.
        self.running_cost = ca.Function('ln', [x, xr, Q, u, R], [ln])

        # Calculate terminal cost, e'*P*e
        V = ca.mtimes(ca.mtimes(e_vec.T, P), e_vec)
        self.terminal_cost = ca.Function('V', [x, xr, P], [V])

    #SLACK HERE!!---------------------------------------------------------------------------------------------------------
    def set_slack_cost(self):
        S = ca.DM.eye(self.Nx) * 500
        slack = ca.MX.sym('lmbd', self.Nx)

        slack_cost_fun = ca.mtimes(slack.T, ca.mtimes(S,slack))
        self.slack_cost = ca.Function('slk', [slack], [slack_cost_fun])

    def solve_mpc(self, x0, u0=None):
        """
        Solve the optimal control problem

        :param x0: starting state
        :type x0: np.ndarray
        :param u0: optimal control guess, defaults to None
        :type u0: np.ndarray, optional
        :return: predicted optimal states and optimal control inputs
        :rtype: ca.DM
        """

        # Initial state
        if u0 is None:
            u0 = np.zeros(self.Nu)
        if self.x_sp is None:
            self.x_sp = np.zeros(self.Nx * (self.Nt + 1))

        # Initialize variables
        self.optvar_x0 = np.full((1, self.Nx), x0.T)

        # Initial guess of the warm start variables
        self.optvar_init = self.opt_var(0)
        self.optvar_init['x', 0] = self.optvar_x0[0]

        solve_time = -time.time()

        param = ca.vertcat(x0, self.x_sp, u0)
        args = dict(x0=self.optvar_init,
                    lbx=self.optvar_lb,
                    ubx=self.optvar_ub,
                    lbg=self.con_lb,
                    ubg=self.con_ub,
                    p=param)

        # Solve NLP
        sol = self.solver(**args)
        status = self.solver.stats()['return_status']
        optvar = self.opt_var(sol['x'])

        solve_time += time.time()
        print('MPC - CPU time: %f seconds  |  Cost: %f  |  Horizon length: %d ' % (solve_time, sol['f'], self.Nt))

        return optvar['x'], optvar['u'] #obtain all predicted states and control inputs

    def mpc_controller(self, x0, t):
        """
        MPC controller wrapper.
        Gets first control input to apply to the system.

        :param x0: initial state
        :type x0: np.ndarray
        :return: control input
        :rtype: ca.DM
        """
        if self.trajectory_tracking:
            x_traj = self.model.get_trajectory(t, self.Nt + 1, self.fw_propagating)
            x_sp = x_traj.reshape(self.Nx * (self.Nt + 1), order='F')
            self.set_reference(x_sp)
        _, u_pred = self.solve_mpc(x0) #solve mpc for a reference we have set, reference set outside of function, obtain pre

        # Calculate error to first state
        error = self.calculate_error(x0, self.x_sp[0:13])

        return u_pred[0], error

    def astrobee_sim_controller(self, x0, xh):
        """
        Controller to be used in the Astrobee simulator

        :param x0: [description]
        :type x0: [type]
        :param xh: [description]
        :type xh: [type]
        :return: [description]
        :rtype: [type]
        """
        x_traj = self.model.forward_propagate(xh, self.Nt + 1, radius=0.5)
        x_sp = x_traj.reshape(self.Nx * (self.Nt + 1), order='F')
        self.set_reference(x_sp)
        _, u_pred = self.solve_mpc(x0)

        # Calculate error to first state
        error = self.calculate_error(x0, x_traj[:, 0])

        return u_pred[0], error

    def calculate_error(self, x, xr):
        """
        Calculate error, used for logging

        :param x: [description]
        :type x: [type]
        :param xr: [description]
        :type xr: [type]
        :return: [description]
        :rtype: [type]
        """

        # Prepare xr
        xr = xr.reshape((xr.shape[0], 1))

        # Prepare variables
        p = x[0:3]
        v = x[3:6]

        pr = xr[0:3]
        vr = xr[3:6]

        # Calculate errors
        ep = p - pr
        ev = v - vr
        q = x[6:10]
        qr = xr[6:10]

        w = x[10:]
        wr = xr[10:]

        eq = ca.DM.ones(3, 1) * (1 - ca.mtimes(qr.T, q)**2)*(180/np.pi)
        ew = w - wr
        #e6 = (1-ca.mtimes(qr.T, q)**2)
        #eq = ca.DM.ones(3,1)*2*np.arccos(np.sqrt(-e6 + 1))

        return np.concatenate((ep, ev, eq, ew)).reshape((12, 1))

    def set_reference(self, x_sp):
        """
        Set the controller reference state

        :param x_sp: desired reference state
        :type x_sp: np.ndarray
        """
        self.x_sp = x_sp

    def set_forward_propagation(self):
        """
        Helper function to set reference forward propagation
        based of current state and velocity measurements.
        """
        self.fw_propagating = True
