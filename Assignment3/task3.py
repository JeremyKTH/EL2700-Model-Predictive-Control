...
ctrb_discrete = control.ctrb(Ad, Bd)
ctrb_rank_discrete = np.linalg.matrix_rank(ctrb_discrete)   
print(ctrb_rank_discrete)
...
Q_coefficients[6:9] = 2     #--> decrease angle deviation
# Q_coefficients[9:12] = 0
R_coefficients[0:3] = 1.5   #--> decrease force input
R_coefficients[3:6] = 7     #--> decrease torque input
...
Q_diag = np.vstack((np.ones((3, 1)) * 20, np.zeros((3, 1))))
R_diag = np.vstack((np.ones((3, 1)) * 20))
